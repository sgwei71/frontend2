...
const CenterProgress = ({ width, centerCode, progress, tasks, taskDetail, taskDetailPosition, taskDetailVisible, taskDetailRef, handleTaskDetail }) => {
  ...
  .map((task, i) => (
    ...
    >
      <span className="w-[35%] max-sm:w-[42%] truncate text-2xl text-black font-bold max-sm:text-base">{task.stageName}</span>

      <div className="w-[45%] max-sm:w-[38%] bg-gray-300 h-7 my-auto rounded-full shadow-inner-all-md max-sm:w-full">
        <div
